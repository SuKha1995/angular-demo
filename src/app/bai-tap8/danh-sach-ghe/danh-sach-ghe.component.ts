...
mangGhe: Ghe[] =[]
  ...
  datGhe(gheDangDat:any){
    if(gheDangDat.TrangThai){
     ...
     this.mangGheDangDat = this.mangGheDangDat.filter((item)=> item.SoGhe !== gheDangDat.SoGhe)
    }
    console.log(this.mangGheDangDat)
  }

  huyGhe(ghe:any){ //ghe = huyGhe(ghe.SoGhe)
    this.mangGheDangDat = this.mangGheDangDat.filter((item)=>item.SoGhe !== ghe);
